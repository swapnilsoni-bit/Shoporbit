...
const LoginPage = () => {
  ...
  // ✅ NEW - Guest Login with proper auth context
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full border border-green-200 space-y-6">
          ...
          <div className="p-4 bg-green-50 rounded-lg border border-green-200 text-center">
            <p className="text-sm font-semibold text-green-700">
              {user.isGuest ? '👤 Guest Mode' : '✓ Authenticated User'}
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-slate-100 space-y-6">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-3 rounded-lg font-bold hover:shadow-lg disabled:opacity-70 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2 transform hover:scale-105 hover:shadow-lg"
            >
              {loading ? (
                <>
